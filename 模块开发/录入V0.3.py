...
# 扩充数量词正则，包含“双瓶装”、“两瓶装”、“双支装”等
QUANTITY_PATTERN = re.compile(
    r'(双支|两支|2支|\*2|对装|双只|两瓶|两支装|双支装|双瓶装|两瓶装|双包装|两份|双份)',
    re.IGNORECASE
)
...
def main():
    ...
    update_map = {}  # 序号 -> {价格, 生产日期, 是否百亿补贴产品, 失效日期, 备注}
    ...
    # ---------- 更新录入表 ----------
    # 初始化列（如果不存在则添加）
    if '价格' not in df_input.columns:
        df_input['价格'] = ''
    if '生产日期' not in df_input.columns:
        df_input['生产日期'] = ''
    if '是否百亿补贴产品' not in df_input.columns:
        df_input['是否百亿补贴产品'] = ''
    if '失效日期' not in df_input.columns:
        df_input['失效日期'] = ''
    if '备注' not in df_input.columns:
        df_input['备注'] = ''

    # 将需要更新的列转为 object 类型，防止类型不兼容
    ...
    # 价格列保留两位小数（仅对数值）
    df_input['价格'] = pd.to_numeric(df_input['价格'], errors='coerce').round(2)
