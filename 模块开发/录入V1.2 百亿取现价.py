...
def compute_candidate_price(row):
    """
    根据商品标签和价格列计算候选价格。
    百亿补贴优先级：规格价*1.1 > 现价*1.1 > 原价（不加价） > 规格价（兜底）
    其他标签：规格价 > 现价 > 原价
    """
    label = str(row.get('商品标签', '')).strip()
    ...
    if label == '百亿补贴':
        # 优先规格价，其次现价，两者都有效则乘1.1
        if not pd.isna(spec_price):
            return spec_price * 1.1
        elif not pd.isna(curr_price):
            return curr_price * 1.1
        elif not pd.isna(orig_price):
            return orig_price          # 不加价
        else:
            return spec_price          # 最终兜底（可能NaN）
    else:
        # 非百亿补贴保持不变
        if not pd.isna(spec_price):
            return spec_price
...
def select_best_record(records, last_price_map=None):
    ...
    if label_col in records.columns:
        ...
        records['_label_score'] = records[label_col].apply(
            lambda x: label_map.get(str(x).strip(), 0)
        )
    else:
        records['_label_score'] = 0
    ...
    records_sorted = records.sort_values(
        by=['_label_score', '_has_date', '_parsed_date', '_final_price', '_spec_priority'],
        ascending=[False, False, False, True, False],
        na_position='last'
    )
    # ====== 修改点：使用 .copy() 创建明确副本 ======
    best = records_sorted.iloc[0].copy()

    # ====== 新增：保存原始百亿补贴价格（未加价） ======
    label = str(best.get('商品标签', '')).strip()
    if label == '百亿补贴':
        # 优先取规格价，其次现价，不取原价
        orig_price = normalize_price(best.get('规格价格'))
        if pd.isna(orig_price):
            orig_price = normalize_price(best.get('现价'))
        best['_original_bai_price'] = orig_price if not pd.isna(orig_price) else np.nan
    else:
        best['_original_bai_price'] = np.nan
...
def main():
    ...
    # <<< 新增：确保录入表中存在“原始百亿补贴价格”列 >>>
    if '原始百亿补贴价格' not in df_input.columns:
        df_input['原始百亿补贴价格'] = ''
    ...
    for seq, group in grouped:
        ...
        update_map[seq] = {
            ...
            '失效日期': new_exp_date,
            '备注': remark,
            '原始百亿补贴价格': best.get('_original_bai_price', np.nan)
        }

    # ---------- 更新录入表 ----------
    for col in ['价格', '生产日期', '是否百亿补贴产品', '失效日期', '备注', '原始百亿补贴价格']:
    ...
    for idx, row in df_input.iterrows():
        ...
        if seq in update_map:
            ...
            # <<< 新增：写入原始百亿补贴价格 >>>
            df_input.at[idx, '原始百亿补贴价格'] = updates['原始百亿补贴价格']
    ...
    # ---------- 新增：价格范围校验（基于“调整价格范围”两列） ----------
    range_high_col = '调整价格范围（最高）仅限通姆平台参考'
    range_low_col  = '调整价格范围（最低）仅限通姆平台参考'
    if range_high_col in df_input.columns and range_low_col in df_input.columns:
        # 转换为数值
        df_input[range_high_col] = pd.to_numeric(df_input[range_high_col], errors='coerce')
        df_input[range_low_col]  = pd.to_numeric(df_input[range_low_col], errors='coerce')
        new_highlights = []
        for idx, row in df_input.iterrows():
            price = row['价格']
            if pd.isna(price):
                continue
            high = row[range_high_col]
            low  = row[range_low_col]
            if pd.isna(high) or pd.isna(low):
                continue
            a = (high + low) / 2.0
            if price > a:
                # 价格高于 a：超出最低/最高 30% 则标红
                if price < 0.7 * low or price > 1.3 * high:
                    new_highlights.append(idx)
            else:
                # 价格低于或等于 a：超出最低/最高 60 元则标红
                if price < low - 60 or price > high + 60:
                    new_highlights.append(idx)
        # 合并到原高亮列表（去重）
        highlight_indices.extend(new_highlights)
