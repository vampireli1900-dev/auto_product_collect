...
def sort_products_by_priority(products):
    sorted_products = sorted(products, key=lambda p: get_priority(p["tags"]), reverse=True)
    # ====================== 【你要的功能：全是 global 只取前2个】 ======================
    all_global = all(p["tags"] == {"global"} for p in sorted_products)
    if all_global:
        sorted_products = sorted_products[:2]  # 只保留前2个
